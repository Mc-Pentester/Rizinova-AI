...
class ChatAPIView(APIView):
    ...
    def post(self, request):
        ...
        # 2️⃣ RAG principal avec déduplication et fallback sécurisé
        try:
            answer, sources = get_rag_answer(question)
        except Exception:
            # fallback si OpenAI / Pinecone indisponible
        ...
        # 5️⃣ Retourner la réponse avec source
        return Response({
            "conversation_id": conversation.id,
            "answer": answer,
            "sources": sources,
            "source": "rag" if sources else "fallback"
        })
