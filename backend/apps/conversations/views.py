...
class ChatAPIView(APIView):
    ...
    def post(self, request):
        ...
        if cached_answer and not return_pdf:
            return Response({"answer": cached_answer, "sources": [], "source": "cache"})

        # ---------- RAG ----------
        try:
            report_answer, sources, confidence_score = get_rag_answer(
                user_id=user_id,
                query=question,
                plan=plan
            )
        ...
        Message.objects.create(conversation=conversation, content=report_answer, is_user=False)
